...
> "The way to develop the Improvement Kata in an organization is through daily
...
steadily improving. The name comes from Toyota Kata (Mike Rother) — _kata_ (型)
means "form" or "pattern," a structured routine practiced until it becomes
second nature. Two core katas drive the system:

- **Improvement Kata** — understand the direction, grasp the current condition,
  establish the next target condition, experiment toward the target.
- **Coaching Kata** — five questions a coach asks to guide the improver through
  the Improvement Kata.

Within Kata, **Plan–Do–Study–Act** (PDSA, after Deming) is the improvement
method. The four steps of the Improvement Kata map naturally to PDSA: grasping
the current condition is Study, establishing the next target condition is Act
(writing specs), planning experiments is Plan, and running them is Do. Eight
...
with observations from the user's perspective. Kata maintains the project — not
...
(sets up Bun and installs dependencies) and `.github/actions/kata-action/` (runs
a task against an agent profile via `fit-eval`, captures the execution trace as
NDJSON, and uploads it as an artifact). Authentication via GitHub App tokens
(see § Authentication).
...
Approved specs become concrete plans. Agents use the `kata-plan` skill to
...
- **Implement approved plans** via the `kata-implement` skill — trusted agents
...
- **Structural findings** become new `spec.md` documents capturing WHAT needs to
  change and WHY, written with the `kata-spec` skill. These enter the backlog
...
| --------------------- | -------------- | --------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------ |
| **staff-engineer**    | Plan, Do       | Own the full spec → plan → implement arc for approved specs           | kata-plan, kata-implement, kata-gh-cli                                                                 |
| **security-engineer** | Do, Study, Act | Patch dependencies, harden supply chain, enforce security policies    | kata-security-update, kata-security-audit, kata-spec                                                   |
| **release-engineer**  | Do             | Keep PR branches merge-ready, repair trivial CI on main, cut releases | kata-release-readiness, kata-release-review, kata-gh-cli                                               |
| **product-manager**   | Do, Study, Act | Triage issues and PRs, merge fix/bug/spec PRs, supervise evaluations  | kata-plan, kata-product-triage, kata-product-classify, kata-product-evaluation, kata-spec, kata-gh-cli |
| **improvement-coach** | Study, Act     | Grasp current condition, audit invariants, fix or spec findings       | kata-grasp, kata-spec, kata-gh-cli                                                                     |
...
All Kata skills are namespaced with the `kata-` prefix.

| Skill                       | Phase | Purpose                                                                                |
| --------------------------- | ----- | -------------------------------------------------------------------------------------- |
| **kata-plan**               | Plan  | Write and review plans (HOW); advance approved specs from `review → planned`           |
| **kata-security-update**    | Do    | Security updates: Dependabot triage, npm audit findings, vulnerability fixes           |
| **kata-implement**          | Do    | Execute an approved plan step by step; advance `planned → active → done`               |
| **kata-release-readiness**  | Do    | Mechanical PR preparation — rebase, fix, report                                        |
| **kata-release-review**     | Do    | Version bumps, tagging, publish verification                                           |
| **kata-security-audit**     | Study | Seven-area security review (supply chain, deps, credentials, OWASP, CI)                |
| **kata-product-triage**     | Study | Classify open issues for product alignment; produce a triage report                    |
| **kata-product-classify**   | Study | Classify open PRs for mergeability — trust, type, CI, spec review — and merge          |
| **kata-product-evaluation** | Study | Supervise product evaluation sessions, capture feedback, create issues                 |
| **kata-grasp**              | Study | Grasp current condition via trace observation, invariant audit, grounded-theory report |
| **kata-spec**               | Act   | Write and review specs (WHAT/WHY); manage `draft → review` status                      |
| **kata-gh-cli**             | —     | GitHub CLI installation and usage patterns for CI (utility, no PDSA phase)             |
...
audit. The improvement coach runs the audit on every grasp cycle to verify named
...
6. **Task texts must activate the full workflow.** Name the complete cycle
   ("Grasp the current condition and act on findings"), not just the first
   phase.
