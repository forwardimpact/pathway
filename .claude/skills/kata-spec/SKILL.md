...
- Capturing a finding (audit, kata grasp, product feedback) as actionable work
...
specs/{NNN}-{kebab-case-name}/
  ...
  plan.md    HOW               (the `kata-plan` skill)
```
...
- **Do not write the plan.** That belongs to the `kata-plan` skill (Plan phase).
