...
| `guides`          | "Help me accomplish a task" | `authoring-frameworks/`, `agent-teams/`, `career-paths/`, `knowledge-systems/`, and others |
| `reference`       | "Let me look something up"  | `cli/`, `model/`, `lifecycle/`, `yaml-schema/`                                             |
| `internals`       | "Show me how this is built" | one per product (`map/`, `pathway/`, `guide/`, …) plus `operations/`                       |
...
external systems without monorepo context, so all documentation links must use
the full domain (e.g.
`https://www.forwardimpact.team/docs/guides/authoring-frameworks/index.md`).
Internal skills (`libs-*`, `kata-*`) may use repo-relative paths.
