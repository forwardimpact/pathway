...
// Integration test: seed reads a roster + walks a raw dir on real disk, so it
// threads a real createDefaultRuntime() with a quiet proc.
...
describe("activity/seed", () => {
  ...
  test("uploads roster and raw files then runs transforms", async () => {
    ...
    await seed({ data, supabase: fake, runtime });
  ...
  test("returns 1 when roster upload fails", async () => {
    ...
    const exitCode = await seed({ data, supabase: fake, runtime });
