 ...
 */
export async function runValidateShacl(runtime) {
  runtime.proc.stdout.write(
    formatHeader("Validating SHACL schema syntax") + "\n\n",
  );
  ...
  try {
    ...
    const files = await runtime.fs.readdir(rdfDir);
    ...
    for (const file of ttlFiles) {
      ...
      const turtleContent = await runtime.fs.readFile(filePath, "utf-8");
