...
function extractPR(payload: Record<string, unknown>) {
  ...
  return [{
    ...
    github_username: (pr.user as Record<string, unknown>)?.login as string || null,
...
function extractReview(payload: Record<string, unknown>) {
  ...
  if (!review) return []
  const repo = (payload as Record<string, Record<string, unknown>>).repository
  const pullRequest = (payload as Record<string, Record<string, unknown>>).pull_request
  return [{
    artifact_type: 'review',
    external_id: `review:${repo?.full_name}#${pullRequest?.number}:${review.id}`,
    repository: repo?.full_name as string,
    github_username: (review.user as Record<string, unknown>)?.login as string || null,
...
function extractCommits(payload: Record<string, unknown>) {
  ...
  return commits.map((commit) => ({
    ...
    repository: repo?.full_name as string,
    github_username: (payload.sender as Record<string, unknown>)?.login as string || null,
