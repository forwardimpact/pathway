...
> `fit-landmark`, `fit-summit`, `fit-eval`, `fit-trace`, and `fit-rc` are
> published to npm and can be installed standalone. `fit-doc` and `fit-terrain`
> are monorepo-only tools that require a full checkout of the
...
Run agents and capture NDJSON traces — for agent evaluations or multi-agent
collaboration. Both CLIs ship in the `@forwardimpact/libeval` npm package.

```sh
npx fit-eval run --task-file=task.md --output=trace.ndjson
npx fit-eval supervise --task-file=task.md --supervisor-profile=judge --agent-profile=coder --output=trace.ndjson
npx fit-eval facilitate --task-file=task.md --facilitator-profile=lead --agent-profiles=eng-a,eng-b --output=trace.ndjson
...
[Trace Analysis](../../guides/trace-analysis/) guides for end-to-end
walkthroughs.
...
Download, query, and analyze agent execution traces produced by `fit-eval`.

```sh
npx fit-trace runs --lookback 7d                                # list recent CI runs
npx fit-trace download <run-id>                                 # downloads to /tmp/trace-<run-id>/structured.json
npx fit-trace split <file> --mode=facilitate                    # split per-source for multi-agent traces
npx fit-trace overview <file>                                   # metadata, summary, turn count, tool usage
npx fit-trace timeline <file>                                   # one line per turn
npx fit-trace stats <file>                                      # token usage and cost
npx fit-trace search <file> 'error|fail' --context 1            # regex search
npx fit-trace tool <file> <ToolName>                            # turns involving a specific tool
npx fit-trace filter <file> --role <role> --tool <name> --error # structural filter
