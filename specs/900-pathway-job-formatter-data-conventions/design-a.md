...
| **K6. Enforcement substrate**             | `validation/level.js` exports three predicates (`checkProfessionalTitleShape`, `checkProfessionalTitleDisjoint`, `checkAutonomyExpectation`). `validateAllData` in `validation.js:65` is the only orchestrator; it already receives `{ levels, disciplines, … }` and iterates levels, so the K3(b) cross-check gets disciplines without a signature change. Two gates close the render path and the authoring path: **(i) Render gate.** A new `loader.loadAndValidate(dataDir)` wraps `loadAllData` + `throwIfErrors(validateAllData(data))`, throwing a `ContractViolationError` (Error subclass with own properties `{ field, value, reason, contractUrl }`) on the first K3/K5 `INVALID_VALUE`. Every render entry point — `bin/fit-pathway.js:323`, `commands/build.js`, `commands/update.js`, `commands/build-packs.js:215`, and `products/pathway/src/lib/yaml-loader.js`'s consumers (`main.js`/`slide-main.js`/`handout-main.js`) — switches from `loadAllData` to `loadAndValidate`. **(ii) Authoring gate.** `runFullValidation` in `schema-validation.js:445` adds a call to `validateAllData(loadedData)` and merges its `errors`/`warnings` into the returned result; `fit-map validate` thereby surfaces K3/K5 violations at authoring time alongside schema and referential errors. The `bin/fit-pathway.js:324` `validateAllData(data);` discarded call is removed (replaced by the gate inside `loadAndValidate`). `loadStandardConfig`/`dev.js`/`build.js` brand-metadata fallback paths are unaffected. | **Wrap only `bin/fit-pathway.js:324`** — `build`/`update`/`build-packs` and the parallel pathway loader all bypass that line (R1/R3 panel). **Inside `loadStandardConfig`** — reads only `standard.yaml`; never sees levels (R3 panel). **Inside `deriveJob` or each leaf formatter** — multiplies the gate into ~5 call sites; misses `--list`. **Schema `pattern`** — cannot express disjointness. **Formatter-side normalisation** — spec forbids. |
...
New method on `DataLoader` in `products/map/src/loader.js`: loads via `loadAllData`, calls `validateAllData(data)`, then invokes a new `throwIfErrors(result, { ruleCodes: ["INVALID_VALUE"], paths: [/professionalTitle$/, /autonomyExpectation$/] })` helper (next to `validateAllData`). The helper throws `ContractViolationError extends Error` with own properties `{ field, value, reason, contractUrl }` set from the first matching error; non-K3/K5 validation errors pass through silently to preserve current render behaviour. Render entry points (`bin/fit-pathway.js`, `commands/build.js`, `update.js`, `build-packs.js`, the parallel `yaml-loader.js`) switch from `loadAllData` to `loadAndValidate`. The existing outer `try/catch` at `bin/fit-pathway.js:319/334` already prints and exits non-zero, so the new error surfaces as a clear CLI message.

**Authoring-path gate** — `runFullValidation`

In `products/map/src/schema-validation.js:445`, `runFullValidation` adds one `const allData = validateAllData(loadedData)` call after the existing referential-integrity step and merges `allData.errors`/`warnings` into the returned result. `fit-map validate` thereby surfaces K3/K5 errors alongside its existing output, with no new code path.
