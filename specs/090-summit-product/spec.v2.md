...
deliver. Now with practiced capability, outcome-weighted growth, and
project-based teams.
...
v2 adds a critical distinction: derived capability (what job profiles say the
team can do) versus practiced capability (what evidence shows the team actually
does). It also connects growth recommendations to GetDX outcomes, so investments
target both structural gaps and lived pain points.
...
**No required external dependencies.** Summit uses only Map data and a team
roster by default. No GitHub App, no webhooks, no Supabase, no LLM calls. It
runs locally, instantly, deterministically. The same inputs always produce the
same output. v2 adds optional evidence and GetDX data consumption — Summit
...
4. **Practiced capability** — when `--evidenced` is passed, display
   `evidenced_depth` alongside `derived_depth` for each skill. Divergence
   reveals skills that exist on paper but aren't practiced, or skills practiced
   beyond what the job profile predicts.

5. **Outcome-weighted growth** — when `--outcomes` is passed, growth
   recommendations incorporate GetDX driver scores. A gap that also shows a
   poorly-scoring GetDX driver gets boosted priority. Managers see both the
...
3. **v2: Project-based YAML** — extends the local YAML format with project teams
   that reference people from the org model by email.
...
# v2: project-based teams (cross-functional)
projects:
  migration-q2:
    - email: alice@example.com      # resolved from teams or org model
    - email: frank@example.com
    - email: grace@example.com
...
truth. Project teams in the `projects` section reference existing team members
by email and resolve their job profiles from the org model or local team
definitions.
...
When `--evidenced` is set, Summit reads evidence aggregates from Map's activity
layer and computes `evidenced_depth`: the count of engineers with at least one
matched evidence row for that skill at working level or above within a lookback
...
v2: When `--evidenced` is passed, risks are assessed against practiced
capability. A skill may not be a single point of failure by derivation (two
people hold it) but becomes one by evidence (only one person actually practices
it).
...
This closes the blind spot where Summit's recommendations were structurally
correct but misaligned with the team's lived experience.
...
v2 adds optional consumption of Map's activity layer (evidence, GetDX scores)
for practiced capability and outcome-weighted growth. The core derivation path
is unchanged. Summit remains fully functional without activity data.
...
v2 creates a deliberate overlap: Summit's `--evidenced` flag reads the same
evidence data that Landmark presents. This is by design — the same data answers
different questions. Landmark asks "what evidence exists?" Summit asks "does
evidence confirm our capability model?"
...
v2: When `--evidenced` is used with `--format json`, output includes both
