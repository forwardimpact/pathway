...
2. **Output format mismatch.** v2 generated fully-transformed database rows as
   ...
   duplicated transform logic. v3 aligns with the ELT pattern (spec 0051):
...
that the ELT Extract step (spec 0051) writes to Supabase Storage.
...
local files. This is the integration point with the ELT pipeline (spec 0051).
...
function from spec 0052 or `transformAll()` from spec 0051) to process the raw
...
```sh
# Via the transform edge function (spec 0052)
curl -X POST http://localhost:54321/functions/v1/transform

# Or via the Node.js transform orchestrator (spec 0051)
