...
| `mintSupabaseAnonKey` | `libraries/libsecret/src/index.js` (edit) | Sibling to `mintSupabaseJwt`. Wraps `generateJWT` with the standard `{role: "anon", iss: "supabase", iat, exp}` payload. Removes the inline anon-payload duplication that today lives in `scripts/env-storage.js:76-79`. |
| `mintSupabaseServiceRoleKey` | same module (edit) | Same shape as above with `role: "service_role"`. Removes the duplicate `service_role` payload literals at `scripts/env-secrets.js:45-53` and `scripts/env-storage.js:37-40`/`72-75`. |
...
| `fit-map activity start` wiring | `products/map/src/commands/activity.js` (edit) | Stops printing `export MAP_SUPABASE_*` lines — every value is already in `.env` after `just env-setup`. Status output narrows to a confirmation that Supabase started on the expected URL. The `start()` function (currently lines 23-39) drops the `formatSubheader("Export these variables…")` block in full; no new exports are added in its place. |
| `activity-start.test.js` rewrite | `products/map/test/activity-start.test.js` (edit) | The four `assert.match(text, /export MAP_SUPABASE_*/)` lines and the order check are replaced with assertions on the new confirmation output. The test moves from "prints the right export commands" to "prints a Supabase-ready confirmation referencing `SUPABASE_URL`." |
...
graph TD
  ...
  ACTSTART -->|--jwt-secret| SBCLI[supabase CLI]
```
...
sequenceDiagram
  ...
  ACT->>ENV: read SUPABASE_JWT_SECRET
  ACT->>SB: supabase start --jwt-secret <secret>
...
| `libraries/libstorage/src/index.js:198-229` | `_createSupabaseStorage(prefix, process)` reads `process.env.SUPABASE_SERVICE_ROLE_KEY` directly. | Add a `config` parameter to `_createSupabaseStorage` and to the public `createStorage(prefix, type, process, config?)`. When the caller does not supply one, the factory constructs `createScriptConfig("storage")` once. |
...
| `products/landmark/src/lib/identity.js:71-104` | Takes `env` (a `process.env`-like object). | Take `config` instead of `env`. Read `config.supabaseUrl()` / `config.supabaseAnonKey()` for refresh; read `config.supabaseJwtSecret()` inside a `try` (see Decision 10). |
...
| `products/landmark/test/lib/sign-test-token.js:14-20` | Test helper reads `process.env.MAP_SUPABASE_JWT_SECRET`. | Accept `secret` as a required argument (no env fallback). Test setup constructs the secret explicitly. The helper is test-only; pulling Config into a `test/lib/` helper would invert the existing dependency. |
...
Two clean-context reviewers evaluated this design. Findings addressed in-line above:

| Severity | Finding | Resolution |
|---|---|---|
| Blocker | Decision 3 prescribed `supabase start --jwt-secret <ours>` — flag does not exist on the Supabase CLI | Switched to `jwt_secret = "env(SUPABASE_JWT_SECRET)"` in `products/map/supabase/config.toml`'s existing `[auth]` block. Decision 3 rewritten with three rejected alternatives |
| Blocker | `products/map/test/activity-start.test.js` asserts the `export MAP_SUPABASE_*` lines; unaccounted for | Added explicit Components row for the test rewrite |
| High | Decision 10 framed `try`-wrapped HMAC verification as best-effort without explaining why it survives the clean-break | Reframed: the `try` is load-bearing for `npx fit-landmark` external users who never run the bootstrap and never get `SUPABASE_JWT_SECRET`; not a shim, the auth-model boundary |
| High | Four orphan modules (`libterrain`, `libstorage`, `landmark/identity`, `landmark/sign-test-token`) don't take a `Config` today; "thin factory" guidance was vague | Added § Per-module injection seams with concrete seam per consumer |
| High | `services/map/server.js` uses property access (`config.supabaseUrl`) not method, plus a `SUPABASE_*` env fallback chain | § Per-module injection seams row 1 calls out the dead property branch + fallback and pins their removal |
| High | `docker-compose.yml` Components row missed `supabase-db` (line 206) `env_file` reference and the `${JWT_SECRET}` interpolations at lines 178, 264 | Rewrote the Components row to enumerate all four compose services touching Supabase, all interpolations, and the deletion of both `.env.storage.*` files |
| Medium | Decision 7 rationale ("hiding the URL adds no security") was weak | Strengthened with the load-bearing argument: `SUPABASE_URL` is published in every anon-keyed client by design |
| Medium | One-home-per-decision violation — `Config` accessor Components row re-explained the credential-set rationale Decision 7 already owns | Trimmed the Components row to capability only; rationale lives in Decision 7 |
| Medium | Live-Postgres test files (12+) read `MAP_SUPABASE_*` directly; not in Components table | Added explicit Components row for the live-test setup migration |
| Medium | `.env.*.example` row claimed "identical block" then referenced URL differences — internally inconsistent | Clarified: variable *names* match; only `SUPABASE_URL` values and adjacent proxy variables differ across the three files |
| Medium | `fit-map activity start` `createSupabaseCli()` singleton blocks per-call secret threading | § Per-module injection seams row 7 names the activity.js refactor |

No blockers remain. All blocker/high/medium findings resolved. Two low-severity notes (storage env-file enumeration, auth-issue mermaid label) are noted but do not require artifact changes — they are covered by the rewritten Components rows.
