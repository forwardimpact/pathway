...
Spec 0240 (guide-npm-package, done) published `@forwardimpact/guide` to npm and
added early `--help`/`--version` handling in `bin/fit-guide.js`. Spec 0230
...
1. **`--help` and `--version` fail despite spec 0240's fix** (#180) — Spec 0240
