...
2. **Edge function deployment.** The ELT pipeline (spec 0051) requires edge
...
3. **Synthetic data loading.** The synthetic data pipeline (spec 0060) needs to
...
- Modifying the activity database schema (same tables from spec 0050).
- Modifying the ELT pipeline logic (see spec 0051).
