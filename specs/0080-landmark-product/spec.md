...
constants from `src/levels.js`. Map's package layout follows spec 0390: all
...
- Summit's growth alignment logic (imported as a library dependency, not a
  ...
  locally). Summit (spec 0090) is currently in draft status and not yet
...
Specs 0350 (end-to-end activity layer) and 0380 (activity seed) brought the
...
- Schema migration:
  ...
  earlier draft under `activity/migrations/` was removed by spec 0350; this is
...
- GitHub extract + transform at
  ...
  Deno/Node implementations called out by spec 0350 have been consolidated.
...
| Prerequisite                                        | Blocks Landmark commands                                                                                                          | Work required                                          |
| --------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------ |
| Summit's `computeGrowthAlignment` export (spec 0090) | `health` growth recommendations (health itself works without Summit — degrades gracefully to driver scores + evidence + comments) | Implement Summit product, export growth function       |
...
spec 0390's `src/`-rooted layout; consumers import via subpath aliases
...
functions (specs 0350 + 0380); Landmark only ever reads.
...
| Cross-product blocker  | Summit `computeGrowthAlignment` (spec 0090, draft)          |
