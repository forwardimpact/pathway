...
- **Map** defines skills, levels, behaviours — the data model. Source under
  ...
  `products/map/package.json` (spec 0390 layout).
...
- **Map data loader** (`@forwardimpact/map` `createDataLoader`, exported from
  ...
  Map's package layout follows spec 0390: all source lives under `src/`, and
...
- **Map activity ingest** — Specs 0350 and 0380 delivered the end-to-end ELT
...
- **Starter data** — `software_engineering` discipline (core: `task_completion`,
  ...
  markers defined on any starter capability — see spec 0080 for the marker
...
- No markers defined — `--evidenced` depends on Guide writing evidence, which
  depends on markers (spec 0080 prerequisite).

**Cross-product dependencies:**

| Dependency                      | Required for                       | Status                                                                                  |
| ------------------------------- | ---------------------------------- | --------------------------------------------------------------------------------------- |
| Landmark (spec 0080)             | Consuming `computeGrowthAlignment` | Draft — Summit should ship first since Landmark depends on it                           |
...
| Map activity layer ingest       | All Map-sourced commands           | Implemented (specs 0350 + 0380) — populated via `fit-map activity seed` or real ETL paths |
