# Plan A — Summit Product (spec 0090)

## Status

Draft plan for spec 0090 (`specs/0090-summit-product/spec.md`). The spec is
...
- Spec 0080 (Landmark) — Landmark imports `computeGrowthAlignment` from Summit,
  but Summit ships without Landmark as a consumer.

## Non-negotiables

- **Follow spec 0390 package layout** (`products/summit/` with only the allowed
...
| `computeGrowthAlignment` export shape diverges from Landmark spec | Medium   | Coordinate with spec 0080. Part 05 uses the exact signature from spec.md:575–584.                                                                 |
...
The plan is "done" (spec 0090 → `done`) when:
