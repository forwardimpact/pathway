...
This is the same diagnosis spec 0130 made about the inner `Libraries` tables
("API-oriented rather than capability-oriented"). Spec 0130 fixed those tables
...
particularly painful — spec 0360 just landed it as the canonical CLI
...
| ----------------------------- | ----------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------- |
| `libs-service-infrastructure` | librpc, libconfig, libtelemetry, libtype, libmock                                | Cohesive, but `libtelemetry`'s logger is used by every CLI and library — burying it under "service infrastructure" hides it from CLI authors. |
...
| ----------------------- | ----------------------------------------------------------------------------------------- | ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `libs-grpc-services`    | librpc, libconfig, libtelemetry, libtype, libmock                                      | 5     | Renamed from `libs-service-infrastructure`. Same membership. Name names the artifact you build (a gRPC service).                                                              |
...
The current columns are `Main API` / `Purpose`. Spec 0130's plan-a §1 already
...
(spec 0390), which validates that `package.json` `main`/`bin`/`exports` fields
