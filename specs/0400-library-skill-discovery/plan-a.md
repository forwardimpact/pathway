...
Implementation plan for [spec 0400](spec.md). Translates the spec's five moves
...
1. **Move the walls** — rename five `.claude/skills/libs-*` directories so the
   six groups match spec 0400's Move 1 table, and update
...
2. **Keep existing body sections (Decision Guide, Composition Recipes, DI
   Wiring) unless the reorganisation forced a change.** Spec 0130 already
...
Run every part on the existing `claude/spec-0400-planning-a4Okz` branch. Each
...
Parts 01 and 02 both depend on this table. Spec 0400 Move 1 is the authoritative
...
- **List verbs, not library names.** Descriptions describe the tasks the group
  performs; naming a library in the first sentence is the anti-pattern spec 0400
  is trying to retire.
...
2. **`libskill` frontmatter description rewrite is a spec requirement even
   though it isn't in a `libs-*` group.** Spec 0400 Move 1 rationale says
...
3. **The CI check walks `Key Exports` across the union of all public subpath
   ...
   `check-exports-resolve.js` pattern (spec 0390) which also walks the `exports`
   map.
...
- Spec 0130 (prior pass on the same files, fixed body content): spec 0130 —
  library skill tables
- Spec 0390 (introduced `check:exports`, the pattern Part 04 reuses):
