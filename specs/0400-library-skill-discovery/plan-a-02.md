...
Part 2 of 4 of [plan-a](plan-a.md) for [spec 0400](spec.md). Depends on Part 01.
...
- **Do not rewrite Decision Guide or Composition Recipes beyond adjustments
  required by the reorganisation.** Spec 0130 already corrected these.
...
- **Body changes:** keep Decision Guide, Composition Recipes, DI Wiring
  ...
  reference from spec 0130's pass.
...
- **`Key Exports`:** read entry files under
  ...
  - libvector: `VectorIndex`, `VectorProcessor` (verify — spec 0130 flagged this
...
- **Members (5):** libui, libformat, libweb, libdoc, libtemplate. libcli and
  ...
  group but they never got a row (spec 0400 §2 "Five libraries are missing or
...
- **`Key Exports`:** read each entry file; the spec 0130 pass already put
...
- **Members (8):** **libcli**, **librepl**, libutil, libsecret, libsupervise,
  ...
  under `libs-system-utilities` but absent from the table per spec 0400 §2).
- **`Key Exports`:**
...
- **Body:** no changes. libskill is pure functions and the existing body is
  accurate after spec 0130's pass; no inner `Libraries` table to restructure.
...
7. **Manual `Key Exports` verification.** Part 04's `check:skill-exports` script
   ...
   names from the old `Main API` column — spec 0130's pass left some stale. When
...
3. **Reserved future extensibility.** If a library grows a new public export
   ...
   but the Capabilities column drifts. This is acceptable; spec 0130 and future
...
5. **Recipe code in Composition Recipes may reference symbols that were renamed
   ...
   catch spec 0130 drift before Part 04 locks the contract.
