...
| K3  | Static cap formula — `maxMisses ∈ [today's class miss count, today's class miss count × 1.5]`, plan-set; never ≥ class size; never auto-ratcheted by CI                                                                   | "Rotation cycle" = one fresh `generateCommentKeys` run with a different RNG state. The 50% upper bound (vs. the candidate-list size, which would be toothless) preserves the cap's role as a structural-change tripwire — a regen that produces 1.5× today's misses still fits, but a keyspace expansion or generator regression breaks the cap. The integer instantiation is plan-set within this band. Auto-tightening adds mechanism for no clear gain — the cap is a tripwire, not a miss-count optimizer. Cap moves are registry edits reviewed by humans. |
