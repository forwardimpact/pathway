...
export function buildEntities(ast, rng) {
  ...
  const orgs = ast.orgs.map(o => ({ ...o, iri: `https://${domain}/org/${o.id}` }))
  const departments = ast.departments.map(d => ({ ...d, iri: `https://${domain}/department/${d.id}` }))
  ...
  const projects = ast.projects.map(p => ({
    ...p, teams: p.teams || [], phase: p.phase || null,
    prose_topic: p.prose_topic || null, prose_tone: p.prose_tone || null,
...
function generatePeople(ast, rng, teams, domain) {
  ...
  for (const team of teams) {
    ...
    people.push(makePerson(name, rng.pick(['L3', 'L4', 'L5']),
      discKeys[rng.weightedPick(discWeights)] || 'software_engineering',
      team, domain, true, null))
  }
  ...
  while (people.length < count && idx < available.length) {
    ...
    people.push(makePerson(name, level, disc, team, domain, false, mgr?.email || null,
...
function makePerson(name, level, discipline, team, domain, isManager, managerEmail, hireDate = '2023-01-15') {
  ...
  return {
    ...
    discipline, level, track: null,
    team_id: team.id, department: team.department,
    is_manager: isManager, manager_email: managerEmail,
    hire_date: hireDate,
