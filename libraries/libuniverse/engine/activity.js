...
const COMMIT_MULT = { baseline: 1.0, moderate: 1.5, elevated: 2.5, spike: 4.0, sustained_spike: 3.5, very_high: 5.0 }
...
const DRIVER_NAMES = Object.fromEntries(ALL_DRIVERS.map(d =>
...
const COMMIT_MSGS = [
  'Add {f} endpoint', 'Fix {f} validation', 'Update {f} tests',
  'Refactor {f} module', 'Optimize {f} performance',
  'Add error handling for {f}', 'Update {f} documentation',
  'Implement {f} caching', 'Add {f} monitoring',
  'Fix race condition in {f}', 'Migrate {f} to new API',
  'Add integration tests for {f}', 'Clean up {f} imports',
]

const PR_TITLES = [
  'Add {f} support', 'Implement {f} workflow', 'Fix {f} edge cases',
  'Upgrade {f} dependencies', 'Refactor {f} architecture', 'Add {f} tests',
]
...
const PROFICIENCY_ORDER = ['awareness', 'foundational', 'working', 'practitioner', 'expert']
...
export function generateActivity(ast, rng, people, teams) {
  const roster = people.map(p => ({
    email: p.email, name: p.name, github_username: p.github_username,
    discipline: p.discipline, level: p.level, track: p.track,
    manager_email: p.manager_email, team_id: p.team_id,
  }))
...
function buildActivityTeams(ast, teams) {
  ...
  for (const org of ast.orgs) {
    result.push({ getdx_team_id: `gdx_org_${org.id}`, name: org.name, is_parent: true,
      parent_id: null, manager_id: null, contributors: 0, reference_id: null, ancestors: [],
  ...
  for (const dept of ast.departments) {
    ...
    result.push({ getdx_team_id: `gdx_dept_${dept.id}`, name: dept.name, is_parent: true,
      parent_id: parentOrg ? `gdx_org_${parentOrg.id}` : null,
      manager_id: null, contributors: dept.headcount, reference_id: null,
  ...
  for (const team of teams) {
    ...
    result.push({ getdx_team_id: team.getdx_team_id, name: team.name, is_parent: false,
      parent_id: parentDeptId,
      manager_id: team.manager ? `gdx_mgr_${team.manager}` : null,
      contributors: team.size, reference_id: null, ancestors,
      last_changed_at: new Date('2025-01-01').toISOString() })
  }
...
function generateSnapshots(ast) {
  ...
  while (y < toY || (y === toY && m <= toM)) {
    ...
    snaps.push({
      snapshot_id: id, account_id: ast.snapshots.account_id,
      last_result_change_at: done,
      scheduled_for: `${y}-${String(m).padStart(2, '0')}-15`,
      completed_at: done, completed_count: 180, deleted_at: null,
    ...
    for (const team of leafTeams) {
      for (const driverId of ALL_DRIVERS) {
          ...
          if (snapDate >= start && snapDate <= end) {
            for (const affect of scenario.affects) {
              if (team.getdx_team_id === `gdx_team_${affect.team_id}`) {
                const dx = (affect.dx_drivers || []).find(d => d.driver_id === driverId)
                if (dx) base += dx.magnitude * ((snapDate - start) / (end - start))
              }
            }
          }
        }
        ...
        scores.push({
          ...
          team_name: team.name, getdx_team_id: team.getdx_team_id,
          is_parent: team.is_parent, parent_id: team.parent_id, ancestors: team.ancestors,
          item_id: driverId, item_type: 'driver', item_name: DRIVER_NAMES[driverId] || driverId,
          response_count: rng.randomInt(5, team.contributors || 10), score,
          contributor_count: team.contributors || 0,
          vs_prev: round1(rng.gaussian(0, 3)), vs_org: round1(rng.gaussian(0, 5)),
          vs_50th: round1(rng.gaussian(2, 5)), vs_75th: round1(rng.gaussian(-3, 5)),
...
function generateWebhooks(ast, rng, people, teams) {
  ...
  for (const team of teams) membersByTeam.set(team.id, people.filter(p => p.team_id === team.id))
  ...
  while (week < globalEnd) {
    ...
    for (const team of teams) {
      ...
      for (let i = 0; i < pushCount; i++) {
        ...
        const cid = generateHash(String(counter), author.name, ts.toISOString())

        webhooks.push({
          delivery_id: `evt-${String(++counter).padStart(8, '0')}`,
          event_type: 'push', occurred_at: ts.toISOString(),
          payload: {
            ref: 'refs/heads/main',
            commits: [{ id: cid + cid, message: rng.pick(COMMIT_MSGS).replace('{f}', feat),
              timestamp: ts.toISOString(), added: [`src/${feat}.js`], removed: [], modified: ['src/index.js'] }],
      ...
      for (let i = 0; i < prCount; i++) {
        ...
        webhooks.push({
          delivery_id: `evt-${String(++counter).padStart(8, '0')}`,
          event_type: 'pull_request', occurred_at: ts.toISOString(),
          payload: {
            action: rng.pick(['opened', 'closed']), number: prNum,
            pull_request: {
              number: prNum, title: rng.pick(PR_TITLES).replace('{f}', feat),
              state: 'open', user: { login: author.github_username },
              created_at: ts.toISOString(), updated_at: ts.toISOString(),
              additions: rng.randomInt(10, 500), deletions: rng.randomInt(0, 100),
              changed_files: rng.randomInt(1, 20), merged: false,
        ...
        if (rng.random() > 0.4) {
          ...
          webhooks.push({
            delivery_id: `evt-${String(++counter).padStart(8, '0')}`,
            event_type: 'pull_request_review', occurred_at: rts.toISOString(),
            payload: {
              action: 'submitted',
              review: {
                ...
                body: rng.pick(PR_BODIES), submitted_at: rts.toISOString(),
              },
    ...
    for (const affect of scenario.affects) {
      ...
      for (const person of teamPeople) {
        for (const skillId of (affect.evidence_skills || [])) {
          const profIdx = Math.min(PROFICIENCY_ORDER.length - 1, Math.max(floorIdx, floorIdx + rng.randomInt(0, 1)))
          evidence.push({
            person_email: person.email, person_name: person.name,
            skill_id: skillId, proficiency: PROFICIENCY_ORDER[profIdx],
            scenario_id: scenario.id, team_id: team.id,
            observed_at: randDate(rng, sStart, sEnd).toISOString(), source: 'synthetic',
          })
        }
      }
    }
...
function randDate(rng, start, end) {
  return new Date(start.getTime() + rng.random() * (end.getTime() - start.getTime()))
}
