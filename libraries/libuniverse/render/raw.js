...
function renderGetDXPayloads(entities, files) {
  ...
  if (entities.activity?.activityTeams) {
    ...
    files.set('getdx/teams.list.json', JSON.stringify({ ok: true, teams: teamsList }, null, 2))
  }
  ...
  if (entities.activity?.snapshots) {
    ...
    files.set('getdx/snapshots.list.json', JSON.stringify({ ok: true, snapshots: snapshotsList }, null, 2))
  }
  ...
  if (entities.activity?.snapshots && entities.activity?.scores) {
    ...
    for (const [snapshotId, scores] of scoresBySnapshot) {
      ...
      files.set(
        ...
        JSON.stringify({ ok: true, snapshot: { team_scores: teamScores } }, null, 2)
      )
    }
  }
