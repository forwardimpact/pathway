...
export function parse(tokens) {
  ...
  function parseStringOrIdent() {
    ...
    throw new Error(`Expected string or identifier at line ${t.line}, got ${t.type} '${t.value}'`)
  }
  ...
  function parseOrg() {
    ...
    while (peek().type !== 'RBRACE') {
      ...
      else throw new Error(`Unexpected '${kw.value}' in org at line ${kw.line}`)
    }
    expect('RBRACE')
    return org
  }
  ...
  function parseTeam(departmentId) {
    ...
    while (peek().type !== 'RBRACE') {
      ...
      else throw new Error(`Unexpected '${kw.value}' in team at line ${kw.line}`)
    }
    expect('RBRACE')
    return team
  }
  ...
  function parseDepartment() {
    ...
    while (peek().type !== 'RBRACE') {
      ...
      else throw new Error(`Unexpected '${kw.value}' in department at line ${kw.line}`)
    }
    expect('RBRACE')
    return { dept, teams }
  }
  ...
  function parsePeople() {
    ...
    while (peek().type !== 'RBRACE') {
      ...
      else throw new Error(`Unexpected '${kw.value}' in people at line ${kw.line}`)
    }
    expect('RBRACE')
    return people
  }
  ...
  function parseProject() {
    ...
    while (peek().type !== 'RBRACE') {
      ...
      else if (kw.value === 'timeline_start') proj.timeline_start = parseDateValue()
      else if (kw.value === 'timeline_end') proj.timeline_end = parseDateValue()
      else if (kw.value === 'prose_topic') proj.prose_topic = parseStringValue()
      else if (kw.value === 'prose_tone') proj.prose_tone = parseStringValue()
      else throw new Error(`Unexpected '${kw.value}' in project at line ${kw.line}`)
    }
    expect('RBRACE')
    return proj
  }
  ...
  function parseDxDrivers() {
    ...
    while (peek().type !== 'RBRACE') {
      ...
      while (peek().type !== 'RBRACE') {
        ...
        else if (kw.value === 'magnitude') driver.magnitude = parseNumberValue()
        else throw new Error(`Unexpected '${kw.value}' in dx_driver at line ${kw.line}`)
      }
      expect('RBRACE')
      drivers.push(driver)
    }
  ...
  function parseAffect() {
    ...
    while (peek().type !== 'RBRACE') {
      const kw = advance()
      if (kw.value === 'github_commits') affect.github_commits = parseStringValue()
      else if (kw.value === 'github_prs') affect.github_prs = parseStringValue()
      else if (kw.value === 'dx_drivers') affect.dx_drivers = parseDxDrivers()
      else if (kw.value === 'evidence_skills') affect.evidence_skills = parseArray()
      else if (kw.value === 'evidence_floor') affect.evidence_floor = parseStringValue()
      else throw new Error(`Unexpected '${kw.value}' in affect at line ${kw.line}`)
    }
    expect('RBRACE')
    return affect
  }
  ...
  function parseScenario() {
    ...
    while (peek().type !== 'RBRACE') {
      ...
      else if (kw.value === 'timerange_start') scenario.timerange_start = parseDateValue()
      else if (kw.value === 'timerange_end') scenario.timerange_end = parseDateValue()
      else if (kw.value === 'affect') scenario.affects.push(parseAffect())
      else throw new Error(`Unexpected '${kw.value}' in scenario at line ${kw.line}`)
    }
    expect('RBRACE')
    return scenario
  }
  ...
  function parseSnapshots() {
    ...
    while (peek().type !== 'RBRACE') {
      const kw = advance()
      if (kw.value === 'quarterly_from') snaps.quarterly_from = parseDateValue()
      else if (kw.value === 'quarterly_to') snaps.quarterly_to = parseDateValue()
      else if (kw.value === 'account_id') snaps.account_id = parseStringValue()
      else throw new Error(`Unexpected '${kw.value}' in snapshots at line ${kw.line}`)
    }
    expect('RBRACE')
    return snaps
  }
  ...
  function parseFramework() {
    ...
    while (peek().type !== 'RBRACE') {
      ...
      else if (kw.value === 'capabilities') fw.capabilities = parseArray()
      else throw new Error(`Unexpected '${kw.value}' in framework at line ${kw.line}`)
    }
    expect('RBRACE')
    return fw
  }
  ...
  function parseContent() {
    ...
    while (peek().type !== 'RBRACE') {
      ...
      else if (kw.value === 'article_topics') content.article_topics = parseArray()
      ...
      else if (kw.value === 'persona_levels') content.persona_levels = parseArray()
      else if (kw.value === 'briefings_per_persona') content.briefings_per_persona = parseNumberValue()
      else if (kw.value === 'notes_per_persona') content.notes_per_persona = parseNumberValue()
      else throw new Error(`Unexpected '${kw.value}' in content at line ${kw.line}`)
    }
    expect('RBRACE')
    return content
  }
  ...
  while (peek().type !== 'RBRACE' && peek().type !== 'EOF') {
    const kw = advance()
    switch (kw.value) {
      case 'domain': ast.domain = parseStringValue(); break
      case 'industry': ast.industry = parseStringValue(); break
      case 'seed': ast.seed = parseNumberValue(); break
      case 'org': ast.orgs.push(parseOrg()); break
      case 'department': {
      ...
      case 'people': ast.people = parsePeople(); break
      case 'project': ast.projects.push(parseProject()); break
      case 'scenario': ast.scenarios.push(parseScenario()); break
      case 'snapshots': ast.snapshots = parseSnapshots(); break
      case 'framework': ast.framework = parseFramework(); break
      case 'content': ast.content.push(parseContent()); break
      default:
        throw new Error(`Unexpected keyword '${kw.value}' at top level, line ${kw.line}`)
    }
  }
