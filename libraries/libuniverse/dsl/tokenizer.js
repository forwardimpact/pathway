...
const KEYWORDS = new Set([
  'universe', 'domain', 'industry', 'seed',
  'org', 'department', 'team', 'name', 'location',
  'parent', 'headcount', 'size', 'manager', 'repos',
  'people', 'count', 'names', 'distribution', 'disciplines',
  'project', 'type', 'phase', 'teams', 'timeline_start', 'timeline_end',
  'prose_topic', 'prose_tone',
  'snapshots', 'quarterly_from', 'quarterly_to', 'account_id',
  'scenario', 'timerange_start', 'timerange_end', 'affect',
  'github_commits', 'github_prs', 'dx_drivers',
  'trajectory', 'magnitude',
  'evidence_skills', 'evidence_floor',
  'framework', 'proficiencies', 'maturities', 'capabilities',
  'content', 'articles', 'article_topics', 'blogs', 'faqs',
  'howtos', 'howto_topics', 'reviews', 'comments', 'courses', 'events',
  'personas', 'persona_levels', 'briefings_per_persona', 'notes_per_persona',
])
...
export function tokenize(source) {
  ...
  while (i < source.length) {
    ...
    // Braces and brackets
    if (source[i] === '{') { tokens.push({ type: 'LBRACE', value: '{', line }); i++; continue }
    if (source[i] === '}') { tokens.push({ type: 'RBRACE', value: '}', line }); i++; continue }
    if (source[i] === '[') { tokens.push({ type: 'LBRACKET', value: '[', line }); i++; continue }
    if (source[i] === ']') { tokens.push({ type: 'RBRACKET', value: ']', line }); i++; continue }
    if (source[i] === ',') { tokens.push({ type: 'COMMA', value: ',', line }); i++; continue }
